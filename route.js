...
const PENDING_ORDER = path.join(__dirname, "database", "session.json")
const credentialPath = path.join(__dirname, "database", "credential.js")
...
function readBotSettings() {
    try {
        // Hapus cache agar require() selalu mengambil data terbaru jika file berubah
        delete require.cache[require.resolve(credentialPath)];
        return require(credentialPath);
    } catch (error) {
        console.error("Gagal membaca file credential:", error);
        return {
            botNumber: "",
            botName: "",
            ownerNumber: "",
...
function writeBotSettings(data) {
    try {
        // Konversi object menjadi string JSON dengan format yang rapi
        const jsonString = JSON.stringify(data, null, 4);
        // Rakit menjadi file javascript valid
        const fileContent = `module.exports = ${jsonString};\n`;
        // Timpa file credential.js
        fs.writeFileSync(credentialPath, fileContent, 'utf8');
        return true;
    } catch (error) {
        console.error("Gagal menulis ke file credential:", error);
        return false;
    }
}
...
routes.post("/api/product/detail", (req, res) => {
    try {
        const { id } = req.body;
        if (!id) {
            return res.status(400).json({
    ...
    */
    ...
    if (
      ...
      return res.status(400).json({
        status: false,
        message:
          "Metode transaksi tidak tersedia pada product ini"
      });
    }
    /*
    |--------------------------------------------------------------------------
    | GET / CREATE GUEST ORDER SESSION
    |--------------------------------------------------------------------------
    */
    let cookie =
      req.cookies?.order_session;
    /*
    |--------------------------------------------------------------------------
...
// BOT CONTROL ROUTES
routes.get("/api/admin/bot/settings", auth, async (req, res) => {
    try {
        const settings = readBotSettings(); // Sesuaikan dengan fungsi pembaca DB/JSON Anda
        res.json({
            status: true,
            data: settings
        });
...
routes.post("/api/admin/bot/edit/bottNumber", auth, async (req, res) => {
    try {
        const { bottNumber } = req.body;
        
        if (!bottNumber) {
            return res.status(400).json({ status: false, message: "Nomor bot tidak boleh kosong" });
        }

        const settings = readBotSettings();
        settings.botNumber = bottNumber;
        writeBotSettings(settings); // Sesuaikan dengan fungsi penulisan DB/JSON Anda

        res.json({
            status: true,
            message: "Nomor bot berhasil diperbarui"
        });
    } catch (err) {
        res.status(500).json({
            status: false,
...
routes.post("/api/admin/bot/edit/botName", auth, async (req, res) => {
    try {
        const { botName } = req.body;
        
        if (!botName) {
            return res.status(400).json({ status: false, message: "Nama bot tidak boleh kosong" });
        }
        ...
        res.json({
            status: true,
            message: "Nama bot berhasil diperbarui"
        });
    } catch (err) {
        res.status(500).json({
            status: false,
...
routes.post("/api/admin/bot/edit/ownerNumber", auth, async (req, res) => {
    try {
        const { ownerNumber } = req.body;
        
        if (!ownerNumber) {
            return res.status(400).json({ status: false, message: "Nomor owner tidak boleh kosong" });
        }

        const settings = readBotSettings();
        settings.ownerNumber = ownerNumber;
        writeBotSettings(settings);

        res.json({
            status: true,
            message: "Nomor owner berhasil diperbarui"
        });
    } catch (err) {
        res.status(500).json({
            status: false,
